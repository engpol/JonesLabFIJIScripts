...
for (i = 0; i < lengthOf(filelist); i++) {
	...
	//Debugging IGNORE  -- - - -   -- - - -   -- - - -   -- - - -   -- - - -   -- - - -   -- - - -   -- - - -   -- - - -   -- - - - 
	  //  setBatchMode(false);
       // exfolder = ("C:" + File.separator + "Users" + File.separator + "olikc" + File.separator + "Desktop" + File.separator + "PhD Research" + File.separator + "Test_Experiment_Folder") 
	   // filelist = getFileList(exfolder) //Get names of all experiment folders in the main folder
       // for (i = 0; i < lengthOf(filelist); i++) {
	//	phantas_dir = ""+exfolder+File.separator+filelist[i]+"Phantast_Output"+"";
	...
	imageCalculator("Multiply create stack", "Corrected_Flo_Image.tiff","PHANTAST"); //Multiply Fluoresence stack by mask so only fluoresence from cell containing regions is included
	selectImage("Corrected_Flo_Image.tiff"); //Select output of multiplication
