...
//2: Divide into NxN grid - can play around - this should be fine at 6x6 it will lead to ridicoulous times otherwise, + seems to be about right deimension size to pickup
...
#@ int(label="N x N grid to divide into (i.e., 2 = 4, 3 = 9 etc.)") n
...
for (rawROI = 0; rawROI < rawROInumber; rawROI++) { //FOR EACH ROI GENERATED BY STARDIST - take measurement and apply relevant filters
    ...
    if (area > 9.5 || area < 3) { //Might have to change Area values depending on your microscopy - don't think it warrants a parameter tbh (not easily distinguishable from within macro) - maybe a variable???
...
contrastpumpfiles = getFileList(exfolder + File.separator + Slide_filelist[slide_number] + File.separator + "Split_Image_Contrast_Pump");
temp_files_to_delete =  ImageFilesOnlyArray(contrastpumpfiles);
for (tempfile = 0; tempfile < lengthOf(temp_files_to_delete); tempfile++) {
File.delete(exfolder + File.separator + Slide_filelist[slide_number] + File.separator + "Split_Image_Contrast_Pump" + File.separator + temp_files_to_delete[tempfile]);
}
}
